...
def main():
    ...
    corpus_path = '%s/lrcorpus_spoken.txt' % dirs
    ...
    with open(corpus_path, 'w', encoding='utf-8') as fc:
        with open(sentence_path, 'w', encoding='utf-8') as fs:
            create(input_path, corpus_path, sentence_path, fc, fs)

    # fe.write('# WRITTEN CORPUS\n')

    input_path = '%s/%s' % (dirs, written_input)
    corpus_path = '%s/lrcorpus_written.txt' % dirs
...
def create(input_path, corpus_path, sentence_path, fc, fs):
